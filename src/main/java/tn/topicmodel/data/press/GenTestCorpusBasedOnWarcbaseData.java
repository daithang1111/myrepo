...
public class GenTestCorpusBasedOnWarcbaseData {
	...
	public static void main(String[] args) throws IOException,
		...
		if (!cmdline.hasOption(OUTPUT_OPTION)
				...
				|| !cmdline.hasOption(CORPUSNAME_OPTION)) {
			HelpFormatter formatter = new HelpFormatter();
			formatter.printHelp(
					GenTestCorpusBasedOnWarcbaseData.class.getCanonicalName(),
					options);
			System.exit(-1);
		}
		...
		for (int i = 0; i < inputTextList.size(); i++) {
			...
			if (label_str != null && label_str.length == 2) {
				...
				if (values.length >= 3) {
					...
					String docTime = getDocTime(values[2]);
					String docId = docFileName.replace(values[2], docTime);
	...
	private static String getDocTime(String longTime) {
		try {
