...
public class PredictMultiClassLogistic {
	private static final Logger LOG = Logger
			.getLogger(PredictMultiClassLogistic.class);
	...
	public static void main(String[] args) throws IOException,
		...
		if (!cmdline.hasOption(OUTPUT_OPTION)
			...
			formatter
					.printHelp(
							PredictMultiClassLogistic.class.getCanonicalName(),
							options);
			System.exit(-1);
		}
		...
		try {
			...
			try {
				...
				for (int i = 0; i < testInstances.numInstances(); i++) {
					double j48Pred = logistic.classifyInstance(testInstances
							.instance(i));
