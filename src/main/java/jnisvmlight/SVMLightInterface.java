 ...
 * 
 * @author Tom Crecelius & Martin Theobald, including a bug fix by George Shaw
 *         (MIT)
 */
					...
					for (int tokencnt = 1; tokencnt < tokens.length; tokencnt++) {
						...
						if (idx >= 0) {
							...
							throw new ParseException(
									"Parse error in FeatureVector of file '"
											+ file.toString()
											+ "' at line: "
											+ cnt
											+ ", token: "
											+ tokencnt
											+ ". Could not estimate a \"int:double\" pair ?! "
											+ file.toString()
											+ " contains a wrongly defined feature vector!",
									0);
						}
					}
					if (dimlist.size() > 0) {
						...
						for (int i = 0; i < dimlist.size(); i++) {
							dimarray[i] = new Integer((String) dimlist.get(i))
									.intValue();
						}
						for (int i = 0; i < vallist.size(); i++) {
							valarray[i] = new Double((String) vallist.get(i))
									.doubleValue();
						}
						LabeledFeatureVector lfv = new LabeledFeatureVector(
								labelvalue, dimarray, valarray);
						data.add(lfv);
					}
				} else {
					throw new ParseException(
							"Parse error in FeatureVector of file '"
									...
									+ cnt
									+ ". "
									+ " Wrong format of the labeled feature vector?",
							0);
				}
			}
			if (data.size() > 0) {
				...
				throw new ParseException("No labeled features found within "
						+ cnt + "lines of file '" + file.toString() + "'.", 0);
			}
