...
print "SUM OF A ROW OF A:::::::::::::::::", A[:1].sum()
#beta
B=A.transpose()
for k in xrange(K):
    B[k,:] =B[k,:]/B[k,:].sum()
print "SUM OF A ROW OF B::::::::::::::::", B[1,:].sum()

#display
f = file(outfile+".topwords", 'w')
fAll =file(outfile+".allwords",'w')
for k in xrange(K):
    ...
    allWords = np.argsort(A[:, k])[-V:][::-1]
 
    print >>fAll, vocab[anchors[k]],':',
    for w in allWords:
    	print >>fAll, vocab[w],B[k,w],
    print >>fAll, ""
