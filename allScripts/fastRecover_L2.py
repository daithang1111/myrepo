...
def KLSolveExpGrad(y,x,eps, alpha=None):
    ...
    norm_alpha =linalg.norm(alpha)
    norm_alpha =norm_alpha*norm_alpha #THANG
    new_obj = KL(y, log_y, proj) +rLambda* norm_alpha
    y_over_proj = y/proj
    #grad = -dot(x, y_over_proj.transpose())
    normalized_alpha = 2*alpha#THANG /norm_alpha
    grad = rLambda*normalized_alpha-dot(x, y_over_proj)
	...
	if new_obj < eps and tmplambda ==0:
        ...
        old_grad = copy(grad)
        y_over_proj = y/proj
    	normalized_alpha = 2*alpha#THANG /linalg.norm(alpha)
    	grad = rLambda*normalized_alpha-dot(x, y_over_proj)
